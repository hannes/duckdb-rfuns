...
test_that("<lgl> %in% <str>", {
  # <str> %in% <lgl>
  expect_error_rfuns_in(c("TRUE", "true", "false", "FALSE"), c(TRUE))
  expect_error_rfuns_in(c("TRUE", NA_character_), c(NA))
  expect_error_rfuns_in(c(NA_character_), c(NA))

  # <lgl> %in% <str>
  expect_error_rfuns_in(c(FALSE), c("FALSE", "abc"))
  expect_error_rfuns_in(c(FALSE, TRUE, NA), c("TRUE", "5", "abc"))
  expect_error_rfuns_in(c(FALSE), c("TRUE", NA_character_))
  expect_error_rfuns_in(c(NA), c(NA_character_))
})
...
test_that("<timestamp> %in% <str>", {
  ...
  expect_error_rfuns_in(c(now), now_chr)
  expect_error_rfuns_in(c(now, now + 1), NA_character_)
  expect_error_rfuns_in(c(now, now + 1), c(now_chr, NA_character_))
  expect_error_rfuns_in(c(na_time), c(now_chr, NA_character_))
})
...
test_that("<str> %in% <timestamp>", {
  ...
  now_chr <- format(now, "%Y-%m-%d %H:%M:%S %Z")
  expect_error_rfuns_in(c(now_chr), now)
  expect_error_rfuns_in(c(now_chr), na_time)

  expect_error_rfuns_in(c(now_chr, "hello"), c(now, NA))
})
...
test_that(" <date> %in% <string>", {
  ...
  expect_error_rfuns_in(c(today), today_chr)
  expect_error_rfuns_in(c(today, today + 1), NA_character_)
  expect_error_rfuns_in(c(today, today + 1), c(today_chr, NA_character_))
})
...
test_that(" <string> %in% <date>", {
  ...
  expect_error_rfuns_in(c(today_chr), today)
  expect_error_rfuns_in(c(today_chr, "hello"), na_date)
  expect_error_rfuns_in(c(today_chr, "hello"), c(today, na_date))
})
