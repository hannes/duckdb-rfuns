...
test_that(r"(<date> > <date> :: Sys.Date() > Sys.Date())", {
  con <- local_con()
...
test_that(r"(<date> > <date> :: Sys.Date() > Sys.Date() + 1)", {
  con <- local_con()
...
test_that(r"(<date> > <date> :: NA_real_ > Sys.Date())", {
  con <- local_con()
  in_df <- tibble::tibble(x1 = NA_real_, x2 = as.Date("2024-02-21"))
  in_rel <- duckdb:::rel_from_df(con, in_df)
  out_rel <- duckdb:::rel_project(
    in_rel,
    list(duckdb:::expr_function(
...
test_that(r"(<date> > <date> :: Sys.Date() > NA_real_)", {
  con <- local_con()
  in_df <- tibble::tibble(x1 = as.Date("2024-02-21"), x2 = NA_real_)
  in_rel <- duckdb:::rel_from_df(con, in_df)
  out_rel <- duckdb:::rel_project(
    in_rel,
    list(duckdb:::expr_function(
