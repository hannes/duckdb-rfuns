...
if [[ $7 = 'true' ]]; then
  if [[ $4 == wasm* ]]; then
    ...
    aws s3 cp $ext.compressed s3://$5/$1/$2/$3/$4/$1.duckdb_extension.gz --acl public-read --debug
  fi
fi
...
if [[ $6 = 'true' ]]; then
  if [[ $4 == wasm* ]]; then
    ...
    aws s3 cp $ext.compressed s3://$5/$3/$4/$1.duckdb_extension.gz --acl public-read --debug
  fi
fi
